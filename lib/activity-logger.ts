...
export class ActivityLogger {
  ...
  // Async version that doesn't block the main flow
  logActivityAsync(entry: ActivityLogEntry): void {
    // Fire and forget - don't await this
    this.logActivity(entry).catch((error) => {
...
export const logResourceCreated = (
  ...
  const activityType =
    activityTypeMap[resourceType] || ActivityType.USER_CREATED;
...
export const logResourceUpdated = (
  ...
  const activityType =
    activityTypeMap[resourceType] || ActivityType.USER_UPDATED;
...
export const logResourceDeleted = (
  ...
  const activityType =
    activityTypeMap[resourceType] || ActivityType.USER_DELETED;
...
export function withActivityLogging(
  ...
  return function (handler: Function) {
    return async function (req: any, res: any, ...args: any[]) {
      ...
      if (res.status < 400) {
        await activityLogger.logActivityFromRequest(
          req,
          activityType,
          description,
          {
