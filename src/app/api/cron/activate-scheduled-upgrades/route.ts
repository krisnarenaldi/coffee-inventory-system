...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Found ${readySubscriptions.length} subscriptions ready for upgrade`);
    ...
    for (const subscription of readySubscriptions) {
      try {
        ...
        if (!newPlan) {
          console.error(`❌ New plan ${subscription.intendedPlan} not found for subscription ${subscription.id}`);
          continue;
        }
    ...
    console.log(`Found ${scheduledTransactions.length} scheduled transactions to process`);
    ...
    for (const transaction of scheduledTransactions) {
      try {
        ...
        if (!subscription) {
          console.error(`❌ No subscription found for tenant ${transaction.tenantId}`);
          continue;
        }

        // Check if current period has ended
        if (subscription.currentPeriodEnd > now) {
          console.log(`⏳ Subscription for tenant ${transaction.tenantId} not ready yet (ends ${subscription.currentPeriodEnd})`);
          continue;
        }

        // Calculate new period dates
        const startDate = subscription.currentPeriodEnd;
        const endDate = computeNextPeriodEnd(startDate, transaction.subscriptionPlan.interval);
        ...
        console.log(`✅ Activated scheduled transaction ${transaction.id} for tenant ${transaction.tenantId}`);
        processedCount++;

      } catch (error) {
        console.error(`❌ Error processing scheduled transaction ${transaction.id}:`, error);
      }
    }

    console.log(`🎉 Scheduled upgrade activation completed. Processed: ${processedCount} upgrades`);
