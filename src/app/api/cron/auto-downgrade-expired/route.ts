...
export async function POST(request: NextRequest) {
  try {
    ...
    // Exclude subscriptions already scheduled for downgrade (handled by process-scheduled-downgrades)
    const expiredSubs = await prisma.subscription.findMany({
      where: {
        currentPeriodEnd: { lt: graceDeadline },
        status: { not: "PENDING_CHECKOUT" as any },
        plan: { name: { not: "Free" } },
        cancelAtPeriodEnd: false, // Only process non-scheduled downgrades
      },
    ...
    if (!expiredSubs.length) {
      return NextResponse.json({
        processed: 0,
        message:
          "No unscheduled subscriptions past grace period to auto-downgrade",
      });
    }
