  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error || !result?.ok) {
        // Check if the error is specifically about subscription expiration
        if (
        ...
        if (session?.user?.role === "PLATFORM_ADMIN") {
          ...
          if (currentHost.includes("localhost")) {
            // For localhost development, always redirect to admin.localhost:3000
            ...
            const adminHost =
              "admin." + currentHost.split(".").slice(1).join(".");
            window.location.href = `https://${adminHost}/admin/dashboard`;
          }
          return;
        }
      ...
      // Check if the error might be due to subscription expiration
      // by attempting to validate the user's subscription
      if (tenantSubdomain) {
        try {
          const response = await fetch("/api/subscription/status", {
